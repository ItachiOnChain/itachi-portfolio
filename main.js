...
// Guaranteed safety fallback: dismiss loader after 2.4s
...
const PHASE_WINDOWS = [
  [0.13, 0.17, 0.21, 0.25],   // Silence
  [0.27, 0.31, 0.37, 0.42],   // Awakening
  [0.45, 0.49, 0.63, 0.69],   // Sharingan
  [0.74, 0.79, 0.97, 1.01],   // A Thousand Payments
];
...
/* Smooth in-page navigation with chrome offset */
...
/* ═════════════════════ scroll-driven read-along highlight ═════════════════════ */
const shBlocks = [...document.querySelectorAll('[data-scroll-highlight]')].map(el => {
...
function paintHighlights() {
  ...
  for (const b of shBlocks) {
    ...
    const p = clamp((vh * 0.72 - r.top) / span, 0, 1);
    const n = Math.round(p * b.spans.length);
    if (n === b.lit) continue;
    for (let i = 0; i < b.spans.length; i++) {
      b.spans[i].classList.toggle('lit', i < n);
    }
    b.lit = n;
  }
}
...
if (shBlocks.length) {
  ...
  const queueHighlights = () => {
    if (shQueued) return;
    shQueued = true;
    requestAnimationFrame(() => { shQueued = false; paintHighlights(); });
  };
